 ...
 * js分页标签
 * @author LEE SIU WAH
 * longIt.pager("pager", {pageIndex: 10, pageSize : 15, pageCount : 1500, 
 *									submitUrl : "http://www.jd.com/video.do?pageIndex={0}"});
 * pageIndex:当前页数
 * pageSize:一页显示的数量
 * pageCount:总记录条数
 * submitUrl:提交的Url
	...
	var _longIt = {
		pager : function(id, page){
			...
			if (this.pageCount === 0 || this.pageCount === this.pageSize){
				...
				res.append("<h4><input type='text' id='longIt_page_num' size='2'/></h4><h6>页&nbsp;</h6>");
				res.append("<h2><a href='javascript:void(0);' id='longIt_page_go'>go</a></h2>");
		...
		page_go : function(){
			...
			document.getElementById("longIt_page_go").onclick = function(){
				var num = document.getElementById("longIt_page_num").value;
				if (longIt.trim(num) == "" || isNaN(num)){
					alert("请输入页码!");
				}
